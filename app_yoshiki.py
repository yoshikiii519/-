...
current_city_code.write("あたなが選択した地域は" + city_code_index + "です。") # 用意した箱に選択肢した地域を代入し、表示させる
...
df1 = pd.DataFrame([weather_json['forecasts'][0]['chanceOfRain'].values()], index=["今日"], columns=["朝", "昼", "夕方", "夜"])
df2 = pd.DataFrame([weather_json['forecasts'][1]['chanceOfRain'].values()], index=["明日"], columns=["朝", "昼", "夕方", "夜"])
df3 = pd.DataFrame([weather_json['forecasts'][2]['chanceOfRain'].values()], index=["明後日"], columns=["朝", "昼", "夕方", "夜"])
